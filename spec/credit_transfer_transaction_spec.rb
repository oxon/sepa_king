  ...
  describe :schema_compatible? do
    ...
    context 'for pain.001.001.03.ch.02' do
      ...
      context 'payment type 1' do
        ...
        it 'should succeed for valid attributes' do
          expect(SEPA::CreditTransferTransaction.new({ ch_isr_participation_number: '01-123455-2', reference: '1234567', currency: 'CHF' }.merge(type))).to be_schema_compatible(schema)
        end
        it 'should not succeed for invalid attributes' do
          expect(SEPA::CreditTransferTransaction.new({ iban: 'CH9109000000300083916', ch_iid: '9000' }.merge(type))).not_to be_schema_compatible(schema)
          expect(SEPA::CreditTransferTransaction.new({ ch_bank_account: '1234', ch_bank_postal_account: '80-2-2', ch_iid: '0230' }.merge(type))).not_to be_schema_compatible(schema)
          expect(SEPA::CreditTransferTransaction.new({ ch_code_line: '> 123124sadfa1231231sadface3412', ch_bank_postal_account: '80-2-2', creditor_bank_name: 'UBS' }.merge(type))).not_to be_schema_compatible(schema)
          expect(SEPA::CreditTransferTransaction.new({ iban: 'CH9109000000300083916' }.merge(type))).not_to be_schema_compatible(schema)
      ...
      context 'payment type 2.1' do
        ...
        it 'should not succeed for invalid attributes' do
          ...
          expect(SEPA::CreditTransferTransaction.new({ ch_code_line: '> 123124sadfa1231231sadface3412', ch_bank_postal_account: '80-2-2', creditor_bank_name: 'UBS' }.merge(type))).not_to be_schema_compatible(schema)
          expect(SEPA::CreditTransferTransaction.new({ iban: 'CH9109000000300083916' }.merge(type))).not_to be_schema_compatible(schema)
          expect(SEPA::CreditTransferTransaction.new({ ch_isr_participation_number: '01-123455-2', reference: '1234567' }.merge(type))).not_to be_schema_compatible(schema)
          expect(SEPA::CreditTransferTransaction.new({}.merge(type))).not_to be_schema_compatible(schema)
        end
      end
      ...
      context 'payment type 2.2' do
        ...
        it 'should succeed for valid attributes' do
          expect(SEPA::CreditTransferTransaction.new({ iban: 'CH9109000000300083916', ch_iid: '9000' }.merge(type))).to be_schema_compatible(schema)
          expect(SEPA::CreditTransferTransaction.new({ ch_bank_account: '1234', ch_bank_postal_account: '80-2-2', ch_iid: '0230' }.merge(type))).to be_schema_compatible(schema)
          expect(SEPA::CreditTransferTransaction.new({ ch_code_line: '> 123124sadfa1231231sadface3412', ch_bank_postal_account: '80-2-2', creditor_bank_name: 'UBS' }.merge(type))).to be_schema_compatible(schema)
        end
        it 'should not succeed for invalid attributes' do
          expect(SEPA::CreditTransferTransaction.new({ iban: 'CH9109000000300083916' }.merge(type))).not_to be_schema_compatible(schema)
          expect(SEPA::CreditTransferTransaction.new({ ch_postal_account: '01-123455-2' }.merge(type))).not_to be_schema_compatible(schema)
      ...
      context 'payment type 3' do
        ...
        it 'should succeed for valid attributes' do
          expect(SEPA::CreditTransferTransaction.new({ iban: 'CH9109000000300083916' })).to be_schema_compatible(schema)
          expect(SEPA::CreditTransferTransaction.new({ iban: 'CH9109000000300083916' }.merge(type))).to be_schema_compatible(schema)
          expect(SEPA::CreditTransferTransaction.new({ iban: 'CH9109000000300083916', ch_iid: '9000' }.merge(type))).to be_schema_compatible(schema)
        end
        it 'should not succeed for invalid attributes' do
          expect(SEPA::CreditTransferTransaction.new({ ch_bank_account: '1234', ch_bank_postal_account: '80-2-2', ch_iid: '0230' }.merge(type))).not_to be_schema_compatible(schema)
